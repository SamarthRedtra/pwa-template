...
const modifiedForgetPasswordURL = ref(`${baseURL.value}:8001/`);
    ...
    .then(response => {
      response.text().then(result => {
        if (response.status === 200) {
          ...
          responsemessage.value = 'Please setup default Email Account from Settings > Email Account';
        }
        else {
          responsemessage.value = 'You hit the rate limit because of too many requests. Please try after sometime.';
        }
        setTimeout(() => {
